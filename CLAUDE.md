...
- **Недельный замер — суббота.** Элементы ступени 5 живут в Fluent с категорией
  `frequentie_zinnen_weekmeting` и `due_date` на субботу. В будни их в очереди
  быть не должно: 2026-09-16 четыре таких предложения стояли на `critical`
  вместе с правилами дня, сортировались раньше по порядку добавления и открыли
  сессию — Alex остановил её первым же вопросом «разве это то, что надо
  сегодня». Заводя новый элемент на `critical`, проверяй, что он не оттесняет
  правило дня.
- **Порядок дня жёсткий: Anki → мостик → Fluent.** Запуск `/fluent-review` до
  утренней сессии Anki бессмыслен — `vandaag.md` вернёт `Geen nieuwe kaarten`, и
  упражнениям не на чем строиться.
...
- **Первым действием любой Fluent-сессии для Alex — пересобрать мостик:**
  `python3 scripts/anki_vandaag.py --out private/frequentie/vandaag.md`, и
  только потом читать файл. Не читай лежащий файл как есть: он может быть
  вчерашним, а запуск `/fluent-review` — это и есть сигнал «Anki пройден»,
  отдельного сообщения от Alex ждать не нужно. Скрипт stdlib-only, ничего не
  пишет в коллекцию, открытый Anki ему не мешает.
- В файле — слова, у которых **сегодня** было первое повторение, то есть реально
  пройденные, а не запланированные. Прошёл 12 из 20 — в файле 12. Пусто
  (`Geen nieuwe kaarten`) — Anki сегодня не было: скажи об этом и не выдумывай
  слова.
