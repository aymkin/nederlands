...
до конца строки, поэтому `proseWrap` больше не режет текст карточки. До правки
2026-09-21 резал — 58 полей в 34 правилах, молча.
...
### Fluent-сессии Alex (грамматический трек)

Порядок дня, мостик из Anki, наполнение упражнений, ступень задания, проверки
очереди и правила оценки — в скилле `nederlands-fluent-sessie`. Грузить перед
`/fluent-review` и `/fluent-learn`; они нужны только во время сессии, поэтому
лежат под указателем, а не в постоянном контексте.
