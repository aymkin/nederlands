...
Мост между учебными материалами (Link/De Opmaat) и базой интервального
повторения Fluent. Читает активную тему курса, извлекает лексику и грамматику,
записывает карточки в `~/.claude/fluent-data/spaced-repetition.json`.
SM-2-расписание ведёт сам Fluent — импортёр только сеет новые items и
...
Фильтр гейта и `--advance` используют префикс `{course}_t{N}_` чтобы изолировать
карточки одной темы.
...
Скрипт пишет **только** `~/.claude/fluent-data/spaced-repetition.json`. Перед
каждой записью создаётся бэкап:
...
Проверки: `python3 scripts/test_fluent_rebuild_queue.py` — всё через CLI, это
единственный интерфейс скрипта. Главные: `test_lag_detected` воспроизводит сам
лаг, `test_apply_leaves_fsrs_untouched` сторожит нетронутость расписания,
`test_rejects_non_iso_date` — что кривая дата не запишет перекошенные бакеты.
