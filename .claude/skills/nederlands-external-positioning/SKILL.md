...
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Personal FSRS weights beat defaults"                    | `metadata.weights: null` — DEFAULT_W in use, permanently. The weekly optimizer never once activated (guard: ≥400 reviews AND ≥50 new) and was **retired 2026-09-16** after its script was deleted (archaeology 12). No personal weights have ever been fitted, and "beats defaults" would additionally require an evaluation (log-loss/calibration vs DEFAULT_W) that does not exist. |
...
- No optimizer exists: `launchctl list | grep fluent-fsrs` (no output) and
  `ls ~/.claude/plugins/cache/*/fluent/*/.claude/hooks/optimize_weights.py`
